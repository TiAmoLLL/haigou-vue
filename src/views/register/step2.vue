 ...
 * @Author: lzy
 * @Description: test
 * @Date: 2022-08-08 20:44:11
 * @LastEditTime: 2022-08-09 16:28:22
 * @FilePath: \haigou-vue-app\src\views\register\step2.vue
-->
<template>
  <div class="step2-box">
    <van-image class="imgBox"
               width="88"
               height="71"
               :src="img" />
    <div class="tips">我们将以短信或电话的形式将验证码发送给您，请注意接听0575/025/0592/010等开头的电话</div>
    <div class="content">
      <!-- 表单 -->
      <van-form>
        <van-field v-model="telcode"
          ...
          <template #button>
            <van-button type="default"
                        ...
                        @click="sendCodeFn">重新发送{{time}}</van-button>
          </template>
        </van-field>
...
<script>
import Vue from 'vue'
import { Image as VanImage, Form, Field, Notify, Toast } from 'vant'
...
export default {
  ...
  methods: {
    onSubmit() {
      doCheckCode({ telcode: this.telcode }).then((res) => {
        console.log(res)
        if (res.code == 200) {
    ...
    sendCodeFn() {
      doSendMsgCode(localStorage.getItem('tel')).then((res) => {
